...
- **Tidbit:** When a **writer is waiting, new readers are turned away** so the writer can't be starved. This wasn't always true — Java 5 let readers "tag‑team" and starve writers indefinitely; it was fixed in **Java 6**. Also: you can **downgrade** write→read by acquiring the read lock before releasing the write lock, but you can **never upgrade** read→write (instant deadlock).
...
## Phaser
- The flexible offspring of barrier + latch: **parties can register and deregister dynamically**, across many phases.
- Great when the participant count changes over time — a crawl or pipeline where tasks join late and drop out after a phase.
- Trade‑off: more moving parts; you have to manage the phase lifecycle (`arrive`, `arriveAndAwaitAdvance`, `arriveAndDeregister`) with discipline.
- **Tidbit:** Added in **Java 7**, it effectively subsumes both `CyclicBarrier` and `CountDownLatch`, and supports **tiered/tree phasers** to cut synchronization contention when you have very many parties.
...
## CompletionService
- Submit a batch, then consume results **in completion order** rather than submission order — the fastest answers first.
- Perfect for scatter/gather across several endpoints where you want to process each response the moment it lands.
- A clean fan‑out/fan‑in pattern with no per‑`Future` polling.
- **Tidbit:** `ExecutorCompletionService` is deceptively simple inside — it just wraps each task so that, on completion, its `Future` is dropped onto an internal `BlockingQueue` that your `take()` drains. Completion order falls straight out of that.

## AtomicInteger
- **Lock‑free** updates to a single counter or flag via CAS (compare‑and‑set) — no monitor, no blocking.
- The right tool for one hot shared number: a request counter, a retry count, a live gauge.
- Trade‑off: atomicity is **per variable** — the moment you need two fields to stay consistent together, you're back to a lock.
- **Tidbit:** Under *heavy* contention `AtomicInteger` can thrash (every core retrying the same CAS). **`LongAdder`** (Java 8) fixes that by striping the count across cells and summing on read — often dramatically faster for write‑heavy counters. Also beware the **ABA** problem; `AtomicStampedReference` is the antidote.

## Credits
- Concurrency is about **controlled coordination**, not raw speed — correctness first, performance second.
- Reach for the **simplest correct primitive**, then optimize only once you've *measured* contention.
- Design for **cancellation, timeouts, and overload** from day one — they're the requirements that surface in production, not in the demo.
- **Tidbit:** The arc of this whole talk is "less manual control, more structure" — from `wait/notify`, to the j.u.c toolkit, to virtual threads and structured concurrency, each step hands more of the bookkeeping to the platform.
