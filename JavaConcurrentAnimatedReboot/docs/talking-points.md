...
- **Tidbit:** Introduced in **Java 8**, and it is famously **not reentrant** and has **no `Condition`s** — re‑lock the same `StampedLock` on one thread and you deadlock yourself. It aljso doesn't implement the `Lock` interface at all.
